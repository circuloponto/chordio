...
// import types
import { IContext, IMex } from "./types"
...
export default function MySongs({ meX }: IMex) {
  ...
  // let chordObLocalStorage = window.localStorage.getItem('chordObj') || '[]'




  // let inputLocalStorage = window.localStorage.getItem('input') || ''


  // let titleLocalStorage = window.localStorage.getItem('title') || ''
  ...
  const [foundX, setFoundX] = useState<Array<string>>([]);
  ...
  */
  ...
  const handleUpOneTone = (e: { currentTarget: { getAttribute: (arg0: string) => any; } }) => {
    const matrixSharp = [
      'C',
      'C#',
      'D',
    ...
    let parsedNotesSharp: Array<any> = []
    let parsedChord: Array<any> = []

    if (parsedNotes.length > 0) {
      parsedNotesSharp = parsedNotes
        .map((note, index) => {
          ...
          if (note?.length) {
            if (note[0].endsWith('#')) {
              // const index = matrixSharp.indexOf(note);
              // note = matrixFlat[matrixSharp.indexOf(note)];
              // console.log('index from parsedNotesSharp', index);
              result = matrixFlat[matrixSharp.indexOf(note as any)];
            }
          }
          return result
        })
      ...
      parsedChord = foundChords.map((chord) => {
    ...
    arrayLines.forEach((item, i) => {
      ...
      for (let j = 0; j < finalFoundX.length; j++) {
        ...
        if (item.includes(chord)) {
          ...
          if (changedChords) arrayLines[i] = changedChords.join(' ')
  ...
  const handleSave = async () => {
    ...
    if (meX.lists_id.length === 0 || meX.lists_id === 0) {
      ...
      Cookies.set('savedchords', objectToSave as any);
